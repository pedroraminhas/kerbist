...
public class RequestTicketIT extends BaseIT {
	...
	public void testValidRequest() throws Exception {
		...
		final Key serverKey = getKey(VALID_SERVER_PASSWORD, VALID_SERVER_NAME);
	...
	public void testValidDoubleRequest() throws Exception {
		...
		SealedView sealedSessionKey1 = result1.getSessionKey();
		SealedView sealedSessionKey2 = result2.getSessionKey();
		SealedView sealedTicket1 = result1.getTicket();
		SealedView sealedTicket2 = result2.getTicket();
		
		SessionKey sessionKey1 = new SessionKey(SecurityHelper.unseal(SessionKeyView.class, sealedSessionKey1, clientKey));
		SessionKey sessionKey2 = new SessionKey(SecurityHelper.unseal(SessionKeyView.class, sealedSessionKey2, clientKey));
		
		Ticket ticket1 = new Ticket(SecurityHelper.unseal(TicketView.class, sealedTicket1, serverKey));
		Ticket ticket2 = new Ticket(SecurityHelper.unseal(TicketView.class, sealedTicket2, serverKey));
	...
	// Test Helpers -------------------------------------------------------------
	private Key getKey(String password, String salt) throws NoSuchAlgorithmException, InvalidKeySpecException {
		return SecurityHelper.generateKeyFromPassword(password, salt);
	}
